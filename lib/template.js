...
export class Template extends String {
   ...
   * @param {Template<T, Start, End>["replacements"]} [defaults]
   * @returns {Template<T, Start, End>}
   */
  static init(template, tags, defaults) {
    return new Template(template, tags, defaults);
  }
   ...
   */
  constructor(template, tags, defaults = {}) {
    super(template);
  ...
  /**
   * @typedef {GetReplacements<Mapping>} Replacements
   */

  /**
   * @typedef {Partial<Replacements>} PartialReplacements
   */
  ...
  render(replacements = {}, validate = false) {
    ...
    for (const placeholder in this.mapping) {
      const k = this.mapping[placeholder];
      // @ts-ignore: ¯\_(ツ)_/¯
      let value = replacements[k];

      if (!isDefined(value)) {
        value = this.replacements[k];
      }
  ...
  /**
   * @typedef {Expand<GetMapping<Placeholders, Start, End>> &
  ...
  #getInitialReplacements(defaults) {
    ...
    for (const key in this.mapping) {
      ...
      const value = defaults[prop] || null;
...
/**
 * @typedef {string | boolean | number | symbol} Primitive
 */

/**
 * Creates a replacements object with values based on the keys of the input mapping.
 *
 * @template {Record<string, string>} M
 * @typedef {{ [K in keyof M as M[K]]: Primitive | null }} GetReplacements
 */
